...
- `pnpm audit:seo` — after `pnpm build`, walks every prerendered page and reports missing titles/canonicals/og:images, titles over 60 chars, descriptions over 150, wrong `<h1>` counts, heading-level jumps, missing skip links, og:image sizes that don't match the file, non-ISO article dates, and missing or unparsable JSON-LD (`scripts/seo-audit.mjs`). Warns only, never fails. Every route gets its head tags from `pageMetadata()` in `src/lib/site-metadata.ts` — the single owner of canonical/OG/Twitter; it drops the " | Daian Scuarissi" suffix from a `<title>` that would pass 60 chars. Structured data comes from `src/lib/structured-data.ts`, rendered with `<JsonLd />`: Person + WebSite on home, BlogPosting on posts, BreadcrumbList everywhere else. `src/app/sitemap.ts` and `src/app/llms.txt/route.ts` read the same `blogSlugs` list the blog does. Posts without an `image` in frontmatter share `public/blog/og-default.jpg` (1200×630). To keep a page out of the index, set `noindex: true` on its entry in `src/lib/routes.ts` (static routes) or in the post's frontmatter; both the page's robots tag and the sitemap filter read that one flag, and the audit warns if they ever disagree.
