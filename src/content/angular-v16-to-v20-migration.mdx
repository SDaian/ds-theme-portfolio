...
tags: ['angular', 'migration', 'upgrade', 'web-development', 'architecture']
...
- `*ngFor` → `@for` with mandatory `track` expressions
- `*ngIf` → `@if` with optional `@else` blocks
- `*ngSwitch` → `@switch` with `@case` statements
...
- `@Input()` → `input()` with better type safety
- `@Output()` → `output()` with cleaner event handling
